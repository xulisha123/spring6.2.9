			...
			if (singletonObject == null) {
				...
				if (acquireLock && !locked) {
					...
					if (Boolean.TRUE.equals(lockFlag)) {
						// 宽松创建机制： 出现了BeanCurrentlyInCreationException的异常， 等待而非立即失败
				...
				try {
					//  标记正在创建， 如果正在创建会异常：BeanCurrentlyInCreationException
					...
					try {
						...
						while ((singletonObject = this.singletonObjects.get(beanName)) == null) {
							...
							// otherThread == currentThread   如果是同一线程，说明出现了循环依赖或重复创建的情况。  直接异常
							// checkDependentWaitingThreads(otherThread, currentThread) 检查是否存在线程间的依赖等待链。
